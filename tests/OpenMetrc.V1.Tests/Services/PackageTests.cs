        ...
        foreach (var apiKey in sharedFixture.ApiKeys)
            foreach (var facility in apiKey.Facilities)
                do
                    try
                    {
                        var packages = sharedFixture.SafeExecutor(() => apiKey.MetrcService.Packages.GetPackageActiveAsync(
                ...
                do
                    try
                    {
                        var packages = sharedFixture.SafeExecutor(() => apiKey.MetrcService.Packages
                            .GetPackageInactiveAsync(facility.License.Number,
                                DateTimeOffset.UtcNow.AddDays(daysBack), null).Result);
                        if (packages == null) continue;
                        testEndpointResult.WasTested = testEndpointResult.WasTested || packages.Any();
                        foreach (var package in packages)
        ...
        foreach (var apiKey in sharedFixture.ApiKeys)
            foreach (var facility in apiKey.Facilities)
                do
                    try
                    {
                        var packages = sharedFixture.SafeExecutor(() => apiKey.MetrcService.Packages.GetPackageOnHoldAsync(
                            facility.License.Number,
                            DateTimeOffset.UtcNow.AddDays(daysBack), null).Result);
                        if (packages == null) continue;
                        testEndpointResult.WasTested = testEndpointResult.WasTested || packages.Any();
