using OpenMetrc.Tests.V2.Models;
using OpenMetrc.V2;
using System.Diagnostics;
using System.Text.Json;

namespace OpenMetrc.Tests.V2.Fixtures;
    ...
    public async Task LoadFacilities()
    {
        foreach (var key in ApiKeys)
            try
            {
                ...
                var sampleFacilities = facilities.Data?.Where(f => f.IsOwner ?? false).Take(3).ToList() ?? new List<Facility>();
                sampleFacilities.AddRange(facilities.Data?.Where(f => f.FacilityType.CanGrowPlants ?? false).Take(3));
                sampleFacilities.AddRange(facilities.Data?.Where(f => f.FacilityType.CanSellToPatients ?? false).Take(3));
