...
public partial class MetrcService : IPlantBatchClient
{
    ...
    Task IPlantBatchClient.PostPlantBatchCreatePackagesAsync(string licenseNumber, IEnumerable<PostPlantBatchCreatePackagesRequest> body, bool? isFromMotherPlant,
        CancellationToken cancellationToken) =>
        !CheckEndpointAvailability(MethodBase.GetCurrentMethod())
            ? Task.CompletedTask
            : PlantBatchClient.PostPlantBatchCreatePackagesAsync(licenseNumber, body, isFromMotherPlant, cancellationToken);
